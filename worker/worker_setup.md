...
2. **Run the Agent** (Single Camera):
   ```bash
   python worker_agent.py --server http://<SERVER-IP>:8000 --user "worker1" --password "pass123" --camera 0
   ```

3. **Run the Agent (Multi-Camera Mode)**:
   You can now monitor multiple feeds from a single machine using the multi-threaded agent:
   ```bash
   python worker_agent.py --server http://<SERVER-IP>:8000 --user "worker1" --password "pass123" --camera 0 1 --camera-id "gate" "lobby" --location "Main Ent" "Front Desk"
   ```
   *Note: Ensure you wrap multiple IDs or Locations in quotes if they contain spaces.*
