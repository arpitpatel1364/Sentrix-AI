  ...
  grid.innerHTML = cameras.map(c => {
    ...
    <div class="panel" style="padding:1.25rem">
      ...
      <div class="camera-preview-box">
        ${online
          ? `<img class="camera-preview-img" src="/api/stream/${esc(nodeKey)}?token=${State.token}"
...
function showROI(nodeKeyOrCamId) {
  ...
  img.src = `/api/stream/${fullKey}?token=${State.token}`;
          ...
          card.innerHTML = `
            <div style="position:relative">
              <div style="position:absolute;top:8px;left:8px;z-index:10;background:rgba(0,0,0,0.6);color:var(--cyan);font-family:var(--font-mono);font-size:0.58rem;font-weight:700;padding:3px 8px;border-radius:4px;display:flex;align-items:center;gap:5px">
                ...
                <img src="/api/stream/${esc(node.id)}?token=${State.token}" style="width:100%;aspect-ratio:16/9;object-fit:cover;display:block;background:#000"
                     onerror="this.src=''">
            </div>
      ...
      if (matrix) {
        ...
        nodes.forEach(node => {
          ...
          box.innerHTML = `
            ...
            <img src="/api/stream/${esc(node.id)}?token=${State.token}" onerror="this.src=''">`;
...
function showFullScreenStream(nodeId) {
  document.getElementById('tactical-stream-img').src = `/api/stream/${nodeId}?token=${State.token}`;
